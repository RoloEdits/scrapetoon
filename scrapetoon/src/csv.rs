...
pub fn write_series_info(path: &str, series_info: &SeriesInfo) {
    ...
    let cleaned_filename = series_info.title
                                        .replace("/", "")
                                        .replace("<", "")
                                        .replace(">", "")
                                        .replace(":", "")
    ...
    let mut writer = csv::Writer::from_path(final_path).expect("The system cannot find the path specified");
    ...
    for chapter in series_info.chapter_info_list.iter() {
        ...
        writer
        .write_record(&[title, author, genre, total_likes, status, release_day, views, subscribers, rating, chapter_number, likes, date])
        .expect("Couldn't write to file.");
    }
